...
> 由 systems-engineer 填寫（2026-09-15）。本檔同時涵蓋 `docs/templates/任務單.md` 的必填區塊（目標、驗收、回報格式、交叉影響），本任務不另出 `ticket.md`。
> 狀態：草稿（S1–S7 可在 G3.5 拍板前先做；S8 系列必須等 `docs/ui/decision-log.md` 拍板）
> 依據：`docs/spec/game-spec.md` v0.1、`docs/spec/ADR-001-solver.md`、`docs/decision-log.md`、`CLAUDE.md`

---

## 0. 目標與驗收

- **一句話**：製作人在 iPad／手機上打開單一 `index.html`，可以打亂、用手勢或記號鍵轉方塊、撤銷／重做／重設、看計時與 QTM 步數；卡住時看下一步提示，或看電腦用建議解與層先法逐步示範；全程斷網可用。
- **對應 Gate**：G4（製作），完成後交 systems-engineer 架構審查，再進 G4.5。
- **來源**：decision-log 2026-09-15 G0、G2 全部拍板。
...
| S8a | UI 骨架：版面、CSS 3D 方塊、記號鍵、HUD、測試掛鉤 | game-engineer | sonnet | S7＋**G3.5 decision-log** | 序列 | 1.5 |
| S8b | UI 手勢與轉視角 | game-engineer | sonnet | S8a | 與 S8c、S8d 並行 | 1 |
| S8c | UI 提示、示範播放器、求解器狀態 | game-engineer | sonnet | S8a | 與 S8b、S8d 並行 | 1.5 |
| S8d | UI 完成畫面、最佳紀錄、首次教學 | game-engineer | sonnet | S8a | 與 S8b、S8c 並行 | 0.5 |
| S9 | Playwright UI 測試 | game-engineer | sonnet | S8b、S8c、S8d | 序列 | 1 |
...
合計約 12.5 session（推論，依第一款的實際節奏估計）。
...
- **輸入**：`game-spec.md` §10.1–§10.4（內容逐字照抄，數值不得自改）。
- **輸出契約**：四檔為合法 UTF-8 JSON；結構與規格範例完全相同。
- **測試**：`T-DATA-01`～`T-DATA-05`（`T-DATA-05` 的簡體字表由本段建立：至少 40 個常見簡體字，**在測試檔中以 `\uXXXX` 跳脫寫法存放**，避免文件與原始碼出現簡體字；可用「這、個、們、說、設、計、時、數、據、級、轉、動、層、邊、塊、角、復、順、鐘、視、圖、顯、示、鍵、錄、紀、頁、確、認」等字的簡體寫法）。
...
- **輸入**：`game-spec.md` §3.4–§3.6、§4、§6.1–§6.3。
- **輸出契約**：
...
- **測試**：`T-ENG-12`～`T-ENG-24`。`T-ENG-24` 用 Node 讀檔掃描 `src/engine/` 與 `src/solver/`（正規表示式與 `check-forbidden.sh` 相同）。
...
### S8a UI 骨架（sonnet；**等 G3.5 decision-log**）

- **前提**：`docs/ui/decision-log.md` 存在且製作人已拍板。若 decision-log 的版型與 `game-spec.md` §9、§11 的行為規則矛盾 → 停下回報 systems-engineer，不自行取捨。
- **要寫的檔案**：`src/ui/index.template.html`、`src/ui/styles.css`、`src/ui/app.js`、`src/ui/cube-view.js`、`src/ui/controls.js`（覆寫 S7 空殼）。
...
- **輸入**：`game-spec.md` §3.2、§4.1、§6.1–§6.2、§9.4（狀態顯示的位置）、§11；decision-log 與選定版型。
- **輸出契約**：
  - `app.js`：建立 store（`getState`、`dispatch`、`subscribe`；`dispatch` 內呼叫 engine `reduce`，遇到 `REJECT` 只記錄不崩潰）、建立 `solver-client` 並 `start()`、組 `ctx = {root, data, engine, getState, dispatch, subscribe, solver, anim}` 後依序呼叫各模組 `mount(ctx)`；監聽 `visibilitychange` 送 `PAUSE`；`NEW_GAME` 的 seed 產生（§5）；`?test=1` 時建立 `window.__cubeTest`（§12.5）。
  - `ctx.anim`：`turn(viewMove, ms) → Promise`、`rotate(rotation, ms) → Promise`、`busy()`，供手勢與示範共用；動畫中其他輸入被擋。
  - `cube-view.js`：以 CSS 3D 渲染 `viewStickers`；每張貼紙元素帶 `data-sticker`（view 索引）；提供 `ctx.view = {projectAxes(viewIndex) → {right:{x,y}, down:{x,y}}, setDragRotation(matrix), clearDrag(), nearestOrient(matrix) → 0..23, highlight(indices)}`。
  - `controls.js`：36 顆記號鍵（送 `TURN`／`ROTATE`）、打亂、再來一局（局中二次確認）、撤銷、重做、重設、時間（依 `timerRefreshMs` 更新）、步數、打亂字串與「非 WCA 官方打亂」。
  - 所有可點元素 ≥ 44×44 px；兩種視圖。
- **測試**：本段不寫 Playwright；回報附兩種視圖的截圖或 DOM 尺寸掃描結果。
- **回報**：`docs/reports/T-001-S8a.md`（含待補文案清單）。

### S8b UI 手勢與轉視角（sonnet）

- **要寫的檔案**：`src/ui/gesture.js`。
- **不可碰**：S8b 以外的所有 `src/ui/` 檔（需要 `ctx` 新增能力 → 回報 S8a 負責人，經 systems-engineer 裁決）。
- **輸入**：`game-spec.md` §9.2、§9.3；`params.gesture`。
- **輸出契約**：`mount(ctx)`；轉層一律呼叫 `engine.gestureToMove`，方向判定使用 `ctx.view.projectAxes`；放開後吸附送 `SET_ORIENT`；示範中拖曳只傾斜、放開彈回；雙指取消轉層。
- **測試**：可在回報中附手動檢查表；自動測試由 S9 的 `T-UI-05`、`T-UI-06` 負責（需要 S8a 的測試掛鉤）。
- **回報**：`docs/reports/T-001-S8b.md`。

### S8c UI 提示、示範播放器、求解器狀態（sonnet）

- **要寫的檔案**：`src/ui/hint-view.js`、`src/ui/demo-player.js`、`src/ui/demo.css`。
- **不可碰**：其他 `src/ui/` 檔。
- **輸入**：`game-spec.md` §2.3、§7.5、§9.1、§9.4、§9.6；`texts.hint`、`texts.demo`、`texts.solver`、`texts.lbl`、`lbl.json` 的公式名稱。
- **輸出契約**：
  - `hint-view.js`：提示鈕（送 `HINT_REQUEST`，再 `await ctx.solver.requestHint(state)` 並 `dispatch` 結果）、提示記號與層高亮（`hintView`、`hintLayer`）、等待與取消、求解器狀態顯示。
  - `demo-player.js`：兩個示範鈕、播放／暫停／單步／速度／離開、記號列高亮與自動捲動、層高亮（`demoNextLayer`）、層先法 8 段清單（點擊 `DEMO_SEEK`）、步數顯示；播放計時器只在本檔。
- **回報**：`docs/reports/T-001-S8c.md`。

### S8d UI 完成畫面、最佳紀錄、首次教學（sonnet）

- **要寫的檔案**：`src/ui/records.js`、`src/ui/tutorial.js`、`src/ui/overlay.css`。
- **不可碰**：其他 `src/ui/` 檔。
- **輸入**：`game-spec.md` §6.3、§6.4、§11；`texts.result`、`texts.tutorial`；`params.storage`。
- **輸出契約**：`records.js` 訂閱 state，於 `recordEligible` 由 false 變 true 時寫入紀錄並顯示完成畫面；有提示／示範顯示徽章；所有儲存存取包 `try/catch`。`tutorial.js` 在旗標不存在時顯示教學，可略過。
...
- **輸入**：`game-spec.md` §12.5；CLAUDE.md §4 的執行方式（找不到 Playwright 或瀏覽器時印可讀錯誤並以非 0 結束；截圖存 `docs/qa/shots/`）。
- **測試**：`T-UI-01`～`T-UI-14`。
...
| X-7 | S7 ↔ S8a～S8d | `build/bundle.js` 檔案清單；UI 空殼檔 | **所有權轉移**：S7 建立空殼後，各 UI 檔所有權依本表移交（S8a：template、styles.css、app.js、cube-view.js、controls.js；S8b：gesture.js；S8c：hint-view.js、demo-player.js、demo.css；S8d：records.js、tutorial.js、overlay.css）。S7 之後不得再寫這些檔。任何段想新增 UI 檔 → 回報 systems-engineer，由 S7 負責人改清單。 |
| X-8 | S8a ↔ S8b、S8c、S8d | `app.js` 與 `ctx` 介面 | **介面先行**：S8b–S8d 只透過 `mount(ctx)` 接入，**不得修改 `app.js`**；需要 `ctx` 新能力 → 回報，由 S8a 負責人加（屬 S8a 範圍的小修）。三段各自的 CSS 檔分開，避免同檔衝突，因此可並行。 |
...
| 4 觸控優先 | 36 顆記號鍵在手機直向放不下、雙指觸發頁面縮放 | 版型由 G3.5 決定；`T-UI-03`；M-03 |
...
| 版型、按鈕排列 | ui-designer（G3.5） | 進行中（本分派單未讀取 `docs/ui/`） |
| 圖片、音效 | 不需要 | — |
...
- 見 `game-spec.md` §14（三題：建議解示範的按鈕名稱、拖曳視角是否吸附、撤銷是否扣回步數）。三題都會影響 G3.5 版型或文案，請在 G3.5 停點一併回覆；未回覆前 S1 依規格暫定值建檔，S8 系列不得開工。
