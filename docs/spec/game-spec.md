...
## 附錄 D：G4.5 後新增的 UI 測試（S12，2026-09-16）

| 編號 | 驗證內容 |
|---|---|
| T-UI-31 | 首次教學「記號小教室」示範轉動進行中按「跳過教學」，之後 54 張貼紙與 `viewStickers(state)` 完全一致（G4.5 E-2） |
| T-UI-32 | 記號小教室示範 `x` 時，整個方塊一起轉（沒有貼紙在局部轉層群組內、`.cube-world` 在轉動中）（G4.5 E-1） |
| T-UI-33 | 按 `y` 鍵後，整顆旋轉動畫結束時 `.cube-world` 立即回到單位矩陣，不得反向回轉；`orient` 已更新（D-33） |
